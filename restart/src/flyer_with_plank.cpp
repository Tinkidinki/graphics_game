...
void FlyerWithPlank::tick(Thrower* thrower){
    ...
    if (hit_plank(thrower)){
        ...
        thrower->speed.y = 2 * thrower->speed.x * sin_theta * cos_theta
                            + thrower->speed.y * (cos2theta - sin2theta);
        cout << "OLD SPEED:" << thrower->speed.x << "," << thrower->speed.y<<endl;
    }
...
bool FlyerWithPlank::hit_plank(Thrower* thrower){
    ...
    float max_h_dist = this->Rectangle::height/2.0f; //hacky, think of something soon
    float max_v_dist = this->Rectangle::height/2.0f;

    if (h_dist < max_h_dist && v_dist < max_v_dist){
        cout << "HIT PLANK" << endl;
        return true;
    }
    else
